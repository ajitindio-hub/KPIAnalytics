...
<head>
    <meta charset="utf-8" />
    <title><?= $WIFILANTITLE ?> – New Users &amp; ARPU</title>
    ...
    <style>
        /* ═══════════════════════════════════════════════════
           NEW USER ARPU REPORT — Component Styles
...
border-right: 1px solid #dcdcdc;
    ...
    padding-right: 12px !important;
        ...
        /* ── Filter bar (period dropdown + dropdowns) ── */
        .arpu-filter-bar {
            display: flex;
            align-items: center;
        ...
        .arpu-kpi-card.kpi-users::before   { background: #e87722; }
        .arpu-kpi-card.kpi-prev::before    { background: #b35a0f; }
        .arpu-kpi-card.kpi-revenue::before { background: #27ae60; }
        .arpu-kpi-card.kpi-arpu::before    { background: #0891b2; }
        ...
        .arpu-kpi-card.kpi-users   .kpi-icon { background: #fef3ec; color: #e87722; }
        .arpu-kpi-card.kpi-prev    .kpi-icon { background: #fde9d9; color: #b35a0f; }
        .arpu-kpi-card.kpi-revenue .kpi-icon { background: #eafaf1; color: #27ae60; }
        .arpu-kpi-card.kpi-arpu    .kpi-icon { background: #e0f7fa; color: #0891b2; }
...
<body class="page-header-fixed">
    ...
    <div class="page-container row-fluid">
        ...
        <?php
            ...
            $_SESSION['submenulevel1'] = "NewUsersARPU";
                ...
                <div class="arpu-page-wrap">
                    ...
                    <div class="breadcrumb arpu-filter-bar">
                        <div class="arpu-title">
                            <i class="icon-user-add" style="color:#e87722;margin-right:8px;"></i>
                            Total New Users &amp; <span>Average ARPU</span> per User
                        </div>
                    ...
                    <div class="arpu-kpi-row">
                        ...
                        <div class="arpu-kpi-card kpi-users">
                            ...
                            <div class="kpi-label">New Users (Current)</div>
                        ...
                        <div class="arpu-kpi-card kpi-prev">
                            <div class="kpi-icon"><i class="icon-user"></i></div>
                            <div class="kpi-label">New Users (Previous)</div>
                        ...
                        <div class="arpu-kpi-card kpi-arpu">
                            ...
                            <div class="kpi-label">Average ARPU / New User</div>
                            ...
                            <div class="kpi-sub">Revenue ÷ New Users</div>
                        </div>
                    ...
                    <div class="arpu-chart-card">
                        <div class="arpu-chart-header">
                            <div class="ch-title">
                                ...
                                New Users vs ARPU Trend
                            </div>
                            <div class="ch-legend">
                                <div class="leg-item">
                                    <div class="leg-dot" style="background:#e87722;"></div>
                                    <span id="legendCurrent">New Users (Current)</span>
                                </div>
                                <div class="leg-item">
                                    <div class="leg-dot" style="background:#b35a0f;"></div>
                                    <span id="legendPrevious">New Users (Previous)</span>
                                </div>
                                <div class="leg-item">
                                    <div class="leg-dot arpu-line"></div>
                                    <span>ARPU (New Users)</span>
                                </div>
                            </div>
                        </div>
                        <!-- Chart or spinner -->
...
<script type="text/javascript"
        src="<?= $baseurl ?>/<?= $appname ?>/<?= $assetsDir ?>/plugins/daterangepicker/moment.min.js"></script>
    <script type="text/javascript"
        src="<?= $baseurl ?>/<?= $appname ?>/<?= $assetsDir ?>/plugins/daterangepicker/daterangepicker.min.js"></script>
...
jQuery(document).ready(function ($) {
    ...
    function renderKpis(summary) {
        var usersChg = makeBadge(summary.new_users_change_pct);
        ...
        $('#kpiActiveUsers')
            ...
            .css({ height:'', width:'' })
            .text(fmtNumber(summary.new_users));
        ...
        $('#kpiPrevUsers')
            .removeClass('loading arpu-skeleton')
            .css({ height:'', width:'' })
            .text(fmtNumber(summary.previous_new_users));
    ...
    function renderArpuChart(chartData, period) {
        ...
        var curUsers   = chartData.map(function(r) { return r.new_users; });
        var prevUsers  = chartData.map(function(r) { return r.new_users_previous; });
        ...
        /*
         * X-AXIS FIX: Use a SINGLE shared xAxis for bars AND line.
         * The old dual-xAxis approach caused two bugs:
         *   1. Tooltip resolved the date from axisValue which is a numeric
         *      string on xAxis[1], so parseInt("15 Apr") = 15 → wrong index.
         *   2. Labels were skipped (interval:2) making dates appear missing.
         *
         * With one xAxis all three series share the same slot positions,
         * so bars and line points align perfectly without any index tricks.
         * The tooltip dataIndex is always the correct array position.
         */
        var option = {
            tooltip: {
                trigger: 'axis',
                ...
                formatter: function(params) {
                    ...
                    params.forEach(function(p) {
                        if (p.seriesName === 'ARPU (Current)') {
                            ...
                            html += '<span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:#0891b2;margin-right:6px;vertical-align:middle;"></span>';
                            html += '<span style="color:#5a6a80;">ARPU (Current): </span><b>' + fmtRev(p.value) + '</b>';
                            html += '<div style="font-size:11px;color:#9aabb8;margin-top:2px;">Revenue ÷ New Users</div>';
                            html += '</div>';
                        } else {
                            var dotColor = p.seriesName.indexOf('Previous') >= 0 ? '#b35a0f' : '#e87722';
            ...
            /* ✅ Single xAxis — all series share the same date slots */
            xAxis: [
                {
                    type: 'category',
                    boundaryGap: true,
                    data: categories,
                    axisLabel: {
                        /* interval:0 = show every label; rotate for 30-day to avoid overlap */
            ...
            series: [
                {
                    name: 'Current Period',
                    type: 'bar',
                    /* ✅ xAxisIndex:0 — same axis as the ARPU line */
                    xAxisIndex: 0,
                    yAxisIndex: 0,
                    barMaxWidth: 22,
                    barGap: '10%',
                    itemStyle: {
                        color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                            { offset: 0, color: '#f0a060' },
                            { offset: 1, color: '#e87722' }
                        ]),
                        borderRadius: [4, 4, 0, 0]
                    },
                    emphasis: { itemStyle: { color: '#c9601a' } },
                    data: curUsers
                },
                {
                    ...
                    itemStyle: {
                        color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                            { offset: 0, color: '#c89070' },
                            { offset: 1, color: '#b35a0f' }
                        ]),
                        borderRadius: [4, 4, 0, 0]
                    },
                    emphasis: { itemStyle: { color: '#8a430a' } },
                    data: prevUsers
                },
                {
                    name: 'ARPU (Current)',
                    ...
                    emphasis: {
                        itemStyle: {
                            ...
                            borderWidth: 2,
                            shadowBlur: 8,
                            shadowColor: 'rgba(8,145,178,.35)'
                        }
                    },
                    areaStyle: {
                        color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                            { offset: 0, color: 'rgba(8,145,178,.12)' },
                            { offset: 1, color: 'rgba(8,145,178,.01)' }
                        ])
                    },
                    z: 10,
                    data: arpuVals
    ...
    function loadArpuData(period) {
        ...
        $.ajax({
            ...
            success : function(resp) {
                if (!resp || resp.error) {
                    console.error('New User ARPU API error', resp);
                    return;
                }
            ...
            error: function(xhr, status, err) {
                console.error('New User ARPU AJAX error', status, err);
    ...
    $('#arpuRegionFilter').on('change', function() {
        ...
        currentPartner = 'all';
        /* Reset and reload partner list filtered to this region */
